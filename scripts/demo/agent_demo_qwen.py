"""Phase 3 demo — Agent → Text → 3D end-to-end pipeline.

For each user prompt:
  1. Qwen2-VL-7B (text-only mode) refines the casual user description
     into a caption matching the training distribution.
  2. CLIP-L text encoder maps the refined caption to (77, 768) embeddings.
  3. v10 conditional diffusion samples a Tucker latent given the embedding.
  4. Reconstruct SDF, run marching cubes, save .obj.

The agent can later be replaced by Claude / GPT / any LLM — the v10
interface (CLIP-L text embedding) is the only contract.
"""
...
from diffats.sampling import si_sample, unnormalize, reconstruct_sdf, make_schedule
...
OUT_DIR = os.path.join(ROOT, 'outputs', 'phase3_demo')
os.makedirs(OUT_DIR, exist_ok=True)

# 6 user prompts spanning shape categories, what an end user would type.
USER_PROMPTS = [
...
# System instruction for the agent: rewrite casual user text
AGENT_SYSTEM = (
    "You are a 3D CAD geometry describer. The user gives you a casual description of a "
    "mechanical part. Rewrite it as a single 60-120-word geometric caption that exactly "
    "matches the training distribution of a text-to-3D model. "
    "STRICT RULES:\n"
    ...
    "featuring...'.\n"
    "Reply with ONLY the rewritten caption, no preamble."
)
...
def refine_with_agent(model, proc, user_text):
    """Run Qwen2-VL in text-only mode to produce a caption-style description."""
    messages = [
        {'role': 'system', 'content': [{'type': 'text', 'text': AGENT_SYSTEM}]},
        {'role': 'user',   'content': [{'type': 'text', 'text': f'User request: "{user_text}"\nWrite the caption:'}]},
    ]
...
def encode_text(tok, clip_text, captions, device):
    ...
    with torch.no_grad():
        out = clip_text(**inputs).last_hidden_state          # (B, 77, 768)
    return out.float()
...
def main():
    ...
    # 1) Agent (Qwen2-VL-7B, text-only mode)
    print('\n[1/3] loading agent (Qwen2-VL-7B-Instruct) ...', flush=True)
    agent = Qwen2VLForConditionalGeneration.from_pretrained(
        'Qwen/Qwen2-VL-7B-Instruct', torch_dtype=torch.bfloat16, device_map='auto')
    agent_proc = AutoProcessor.from_pretrained('Qwen/Qwen2-VL-7B-Instruct')

    # 2) CLIP-L text encoder
    print('\n[2/3] loading CLIP-L text encoder ...', flush=True)
    ...
    # 3) v10 conditional diffusion
    print('\n[3/3] loading v10 model ...', flush=True)
    ...
    for i, user_text in enumerate(USER_PROMPTS):
        ...
        # Step A: agent refines
        t0 = time.time()
        refined = refine_with_agent(agent, agent_proc, user_text)
        t_agent = time.time() - t0
        print(f'REFINED  : {refined}', flush=True)
        print(f'  [agent: {t_agent:.1f}s]', flush=True)

        # Step B: CLIP encode
        emb = encode_text(clip_tok, clip_text, [refined], device)   # (1, 77, 768)
        emb = emb.unsqueeze(1)                                      # (1, 1, 77, 768)
    ...
    print(f'\n=== TIMING (per prompt avg) ===', flush=True)
    if summary:
        print(f'  agent : {np.mean([s["agent_time_s"]  for s in summary]):.1f}s')
